...
### SAME参数下的pad方式
- ![](1.png)
- 在后面的学习中发现了一个很神奇的问题，发现自己理解又出现了偏差，之前想的是VALID和SAME之间的区别只是在步长不是整数的时候会发生差异，但是发现，即使在剩余的列数能够满足步长的情况下还是会进行补零的操作。这就让我很不解了，完全颠覆了之前的理解，后来又写了程序进行了比较，发现和想的一样，如果是在SAME的模式下，一定是会进行补零的操作的，在补零的时候，按照先在右边补零然后在左边补零，先在下方补零，然后在上方补零，补零的个数直到满足步长的条件为止。
